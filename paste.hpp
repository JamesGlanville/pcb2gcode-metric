...
class paste_exception : virtual std::exception, virtual boost::exception {};
...
{
public:
	PasteProcessor( const string pastefile, const ivalue_t board_width );
	~PasteProcessor();
	...
	void export_ngc( const string of_name,  boost::shared_ptr<Paster> target, bool mirrored, bool mirror_absolute );
	//void export_ngc( const string of_name,  boost::shared_ptr<Cutter> target, bool mirrored, bool mirror_absolute );

	 boost::shared_ptr<const map<int,pasteneedle> > get_needles();
	 boost::shared_ptr<const map<int,icoords> > get_blobs();
...
private:
	...
	boost::shared_ptr< map<int,pasteneedle> > needles;
	boost::shared_ptr< map<int,icoords> > blobs;
